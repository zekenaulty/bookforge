                        ...
                        "scenes": [
                            {
                                ...
                                "location_start_id": "LOC_CITY_GATE_ABC123",
                                "location_end_id": "LOC_CITY_GATE_ABC123",
                                "handoff_mode": "direct_continuation",
                                "constraint_state": "free",
                                "transition_in_text": "At the city gate, the action begins immediately.",
                                "transition_in_anchors": ["city gate", "stone arch", "guard banner"],
                                "seam_score": 10,
...
def _write_outline_report(book_root: Path, overall_status: str, requires_attention: bool = True, severity: str = "error") -> None:
    outline_root = book_root / "outline"
    run_id = "test_run"
    ...
    report = {
        ...
        "reason_codes": [],
        "attempt_usage": {},
        "attention_items": [{"code": "test_issue", "severity": severity, "message": "test"}],
    }
    ...
    (outline_root / "outline_pipeline_report_latest.json").write_text(
        json.dumps(latest_pointer, ensure_ascii=True, indent=2),
        encoding="utf-8",
    )
...
def test_run_blocks_on_error_status_without_bypass(tmp_path: Path) -> None:
    ...
    _write_outline_report(book_root, "ERROR", requires_attention=True, severity="error")
...
def test_run_requires_ack_for_warning_attention(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    book_root = _init_book(tmp_path)
    _write_outline_report(book_root, "SUCCESS_WITH_WARNINGS", requires_attention=True, severity="warning")

    with pytest.raises(ValueError, match="--ack-outline-attention-items"):
        run_loop(workspace=tmp_path, book_id="my_book", steps=1)
